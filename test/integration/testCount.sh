...
if [ "$#" -ne 1 ]; then
    ...
    delta=`awk 'function max(a,b){return a>b?a:b} ; function abs(v) {return v < 0 ? -v : v}{print max( abs($2-1872), int(abs($3-12.531)*10) )}' countresult`
    echo "         NEW RESULT       CACHED RESULT"
    echo "n spots  " ${nspots}   " 1872 " 
    echo "<d>      " ${avgdiam}  " 12.531 "
