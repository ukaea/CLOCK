!------This is a test program to check the background substration in Flat is working correctly.
!*-----------------------------------------------------------------------------------------------------------------------------------
!*    TestFlatten from the Culham LOop Counting Kit (CLOCK), a library for automated feature detection in irradiated transmission electron micrographs
...
!   Any other details
   ...
   integer             ::      noofTests=1 !
   character(len=32)   ::      libName="testFlatten"
   logical             ::      tempCheck
   real(kind=real64)   ::      snr_compare
...
!---    dummy 
   ...
   real(kind=real64)                   ::      img_min,img_irange !min, averge and range of image intensity
   ...
   !----main prog end------
   !test 1, does performing the background subtraction increase the signal to noise ration
   tempCheck=.false.

   if(snr>snr_compare) tempCheck=.true.
   call announceSubTest(libName,"background subtraction",1,noofTests,tempCheck,correctcount)
