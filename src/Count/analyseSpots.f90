...
!*
    ...
    !---    static fields
        real(kind=real64),public                ::      F_THRESH = 3.0d0    !intensity threshold of spot detection
...
!---^^^^^^^^
        ...
        subroutine fitSpots0(img,fitted_g2d,f_auto,maxpix_auto,roi_auto,ss ,bb, ofile)
    !---^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        !---    new code            
            ...
            real(kind=real64),intent(out)                   ::      bb              !   background average intensity
            ...
            real(kind=real64)                   ::      tt,ff    !Background, threshold and foreground intensity
        ...
        !       determine the background noise level                                            
            ...
            !    F_THRESH = 3!(tt-bb)/ss!                   !   auto detects set signal to noise threshold to a sensible default value.                
                F_THRESH = (tt-bb)/ss                      !   auto detects set signal to noise threshold to a sensible default value.                
...
!             
                ...
                if (OPRANDC) then 
                    ...
                    do iy = 0,Ny-1
                        do ix = 0,Nx-1
                            dd = img(ix,iy)
                            if (dd>ff) then
                                img_dbg(1:3,ix,iy) = getRGB_double( COLOURSCALE_MAGMA,1.00d0 ) 
                            else if (dd>tt) then
                                img_dbg(1:3,ix,iy) = getRGB_double( COLOURSCALE_MAGMA,0.67d0 ) 
                            else if (dd>bb) then
                                img_dbg(1:3,ix,iy) = getRGB_double( COLOURSCALE_MAGMA,0.33d0 ) 
                            end if
        ...
        !---    find the spots                     
            ...
            do roi = 1,nroi
                ...
                if (DBG_ROIX==-1) call progressBar( roi,nroi )      !   only output progress bar if bdg mode not on.
            ...
            !---    new fitting code     
                if (FITANDCOUNT) then
                !     DETECTFMIN =(tt-bb) !JPH HACLK (tt-bb)
                !     if(.not.f_auto) then
                !         DETECTFMIN = DETECTFMIN * F_THRESH
                !     end if
                ...
                !---    I think I can op for debug mode only    
                    ! if (OPROI .and. (DBG_ROIX>-1)) then
