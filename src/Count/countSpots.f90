...
!       4.4.0           More work on fitting Gaussians

    character(len=*),parameter      ::      VERSION = "4.4.0"
...
!---        input options
    ...
    real(kind=real64)               ::      max_roi_diameter_factor = 1.0d0 !Holds maxRoiDfact CLA, if 1 a fit to a spot will be rejected if it is larger than the maxium dimesion of its ROI
    logical                         ::      calc_bg_in_rss=.true.   !holds result of  fitBG CLA, if true CLOCK will attmept to account for the any left over background intensity during rss/drss calcualtions
...
!---    read command line arguments
    ...
    call get( cla,"maxRoiDfact",max_roi_diameter_factor ,LIB_CLA_OPTIONAL,"How many times larger a spot can be than the max dimesion of an ROI",3 ) 
    call get( cla,"fitBG",calc_bg_in_rss ,LIB_CLA_OPTIONAL,"      Attempt to account for background intensity during fitting calculations",3 )              
...
!---    welcome        
    ...
    if (hasArgument(cla,"maxRoiDfact"))then
        LIB_G2D_MAX_DIAM=max_roi_diameter_factor
        print*,"   User changed: accepting spot fits diamters that are",LIB_G2D_MAX_DIAM,"times larger than their ROI."
    else
        print*,"   Defaulted to accepting spot fits diamters that are",LIB_G2D_MAX_DIAM,"times larger than their ROI."
    end if

    if (hasArgument(cla,"fitBG")) then
        if (calc_bg_in_rss)then
            LIB_DRAWELLIPSE_RSS_REMOVE_BG=.true.
            print*,"   Attempt to fit back ground during rss calcualtions:",LIB_DRAWELLIPSE_RSS_REMOVE_BG
        else
            LIB_DRAWELLIPSE_RSS_REMOVE_BG=.false.
            print*,"   Attempt to fit back ground during rss calcualtions:",LIB_DRAWELLIPSE_RSS_REMOVE_BG
        end if
    else
        print*,"   Attempt to fit back ground during rss calcualtions, defaulted to:",LIB_DRAWELLIPSE_RSS_REMOVE_BG
    end if
