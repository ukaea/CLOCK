...
!---
   ...
   interface   denoise
       module procedure    denoise64
       module procedure    denoise32
       module procedure    denoise_stripe
   end interface
!---

   logical,public     ::      LIB_LOCALFILTER_DBG = .false.
...
!*      note there is no need to normalise
   ...
   !---    set the value of the private static variable sigma
...
!---^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^                       
   ...
   !---    set up the weighting on the nodes to use.
       ...
       !if (LIB_LOCALFILTER_DBG) print *,"ignoreCount ",ignoreCount
   ...
   !   find matrix A and rhs b
       ...
       call DGESV( 4,1,AA(1:4,1:4),4,ipiv,bb(1:4),4, ierror ) 
           ...
           ! if (LIB_LOCALFILTER_DBG)  print *,"Lib_LocalFilter2d::interpolate55 info - DGESV ",ierror,bb(1:4)
           if (ierror /= 0) then
...
!---
   ...
   recursive subroutine denoise64( f_in,sig,f_out,stdev )
!---^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
!*      find standard deviation of noise, defined as rms difference berween input value and filtered value
!*      and smooth any dodgy pixels with > sig standard deviations. This is for 64 bit reals.
       real(kind=real64),dimension(:,:),intent(in)         ::      f_in
       ...
       if (LIB_LOCALFILTER_DBG) print *,"Lib_LocalFilter2d::denoise64 - info on input stdev,ngoodpix ",stdev,ngoodpix
       ...
       do iy = 1,ny !handle boundaries
           ...
           do ix = 1,nx
               ...
               if ( (f0 /= LOCALFILTER2D_IGNORE).and.(f_in(ix,iy) /= LOCALFILTER2D_IGNORE) ) then
                   deltaf = f0 - f_in(ix,iy)
                   df2sum = df2sum + deltaf*deltaf
                   nGoodPix = nGoodPix + 1
               end if
...
!                    f_local = eoshift( f_local,shift=1,dim=1 )
       ...
       if (LIB_LOCALFILTER_DBG) print *,"Lib_LocalFilter2d::denoise64 - info stdev,ngoodpix ",stdev,ngoodpix," using sig = ",sig
   ...
   !   set f_tmp to be f_in, but now with any dodgy pixel > sig standard deviations set to IGNORE.
       ...
       do iy = 1,ny
           do ix = 1,nx             
               !LIB_LOCALFILTER_DBG = ( ((ix-LIB_LF2D_DBGX)*(ix-LIB_LF2D_DBGX) + (iy-LIB_LF2D_DBGY)*(iy-LIB_LF2D_DBGY) )==0 )
               if ( (f_out(ix,iy) /= LOCALFILTER2D_IGNORE).and.(f_in(ix,iy) /= LOCALFILTER2D_IGNORE) ) then
                   deltaf = f_out(ix,iy) - f_in(ix,iy)
                   if (deltaf*deltaf > stdev*stdev*sig*sig) then
                       f_tmp(ix,iy) = LOCALFILTER2D_IGNORE
                       !if (LIB_LOCALFILTER_DBG) print *,"pixel ",ix,iy," input ",f_in(ix,iy)," output ",f_out(ix,iy)," set to ignore, std dev ",deltaf/stdev
                   end if
               end if                        
           end do
       end do
   ...
   !   set the pixels < sig standard deviations back to the original values - this is a denoise not a smoothing filter.
       if (LIB_LOCALFILTER_DBG) print *,"Lib_LocalFilter2d::denoise64 - info stdev ",stdev, " after removing dodgy pixels  "
       nGoodPix = 0
       df2sum = 0.0d0
       do iy = 1,ny
           do ix = 1,nx                    
               ...
               if ( (f_out(ix,iy) /= LOCALFILTER2D_IGNORE).and.(f_in(ix,iy) /= LOCALFILTER2D_IGNORE) ) then
                   deltaf = f_out(ix,iy) - f_in(ix,iy)
                   if (deltaf*deltaf < stdev*stdev*sig*sig) then
                       !if (LIB_LOCALFILTER_DBG) print *,"pixel ",ix,iy," input ",f_in(ix,iy)," output ",f_out(ix,iy)," set to revert, std dev ",f0/stdev
                       f_out(ix,iy) = f_in(ix,iy)
                       deltaf = 0
                   else
                      !if (LIB_LOCALFILTER_DBG)  print *,"pixel ",ix,iy," input ",f_in(ix,iy)," output ",f_out(ix,iy)," set to flatten, std dev ",f0/stdev
                   end if
                   nGoodPix = nGoodPix + 1
                   df2sum = df2sum + deltaf*deltaf
               else  
       ...
       if (LIB_LOCALFILTER_DBG) print *,"Lib_LocalFilter2d::denoise64 - info stdev,ngoodpix ",stdev,ngoodpix
       ...
       if (LIB_LOCALFILTER_DBG) print *,"Lib_LocalFilter2d::denoise0 - info noise  stdev,ngoodpix,sig ",stdev,ngoodpix,sig
       return
   end subroutine denoise64        
   

   recursive subroutine denoise32( f_in,sig,f_out,stdev )
!---^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
!*      find standard deviation of noise, defined as rms difference berween input value and filtered value
!*      and smooth any dodgy pixels with > sig standard deviations. This is for 32 bit reals.
       real(kind=real32),dimension(:,:),intent(in)         ::      f_in
       real(kind=real64),intent(in)                        ::      sig
       real(kind=real32),dimension(:,:),intent(out)        ::      f_out
       real(kind=real64),intent(out)                       ::      stdev
       ...
       if (LIB_LOCALFILTER_DBG) print *,"Lib_LocalFilter2d::denoise1 - info global stdev,ngoodpix ",stdev,ngoodpix 
       ...
       do iy = 1,ny
           ...
           do ix = 1,nx
               ...
               if (LIB_LOCALFILTER_DBG) then
                   print *,"f_in ", f_in(ix,iy)," f_out = ",f_out(ix,iy)," df = ",f0," nGoodPix ",nGoodPix
                   print *,"" 
               end if         
           !---    slow lines                               
                f_local(-2,-2:2) = f_local(-1,-2:2)
       ...
       if (LIB_LOCALFILTER_DBG) print *,"Lib_LocalFilter2d::denoise1 - info noise  stdev,ngoodpix,sig ",stdev,ngoodpix,sig
           ...
           do ix = 1,nx             
               ...
               if ( (f_out(ix,iy) /= LOCALFILTER2D_IGNORE32).and.(f_in(ix,iy) /= LOCALFILTER2D_IGNORE32) ) then
                   ...
                   if (f0*f0 > stdev*stdev*sig*sig) then
                       ...
                       if (LIB_LOCALFILTER_DBG) print *,"pixel ",ix,iy," input ",f_in(ix,iy)," output ",f_out(ix,iy)," set to ignore, std dev ",f0/stdev
                   end if
               end if                        
           end do
       end do
...
!*      and smooth any dodgy pixels with > sig standard deviations.
       ...
       LIB_LOCALFILTER_DBG = LIB_LF2D_DBGX /= -1
       ...
       if (LIB_LOCALFILTER_DBG) print *,"Lib_LocalFilter2d::denoise_stripe - info global stdev,ngoodpix ",stdev,ngoodpix 
