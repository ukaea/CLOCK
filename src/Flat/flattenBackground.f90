...
!*    Copyright (C) UKAEA 2023 - Jul 2025  Daniel Mason
...
!*      version history
!*      0.1.0       Jul 2025        Changed MaxLikelihood call to new library
...
!---    command line arguments (CLA)
   ...
   logical                             ::      dbg = .false.
   integer                             ::      median_filter=0!=.false.
   real(kind=real64)                   ::      gaussian_blur_filter=-1.0d0
...
!---        
   ...
   call get( cla,"dbg",dbg,LIB_CLA_OPTIONAL,"                output debugging info" ) 
   call get( cla,"med",median_filter,LIB_CLA_OPTIONAL,"                apply median filter with med by med sized window (must be odd) " )  
   call get( cla,"gbf",gaussian_blur_filter,LIB_CLA_OPTIONAL,"                apply gaussian blur filter and define range" )   
...
!---    welcome
   ...
   if (vRC_in /= LIB_CLA_NODEFAULT_I) then
       ...
       print *,"       variable Ridler-Calvard not used"
   end if
...
!---    first step is to tidy up the file a bit
   ...
   !    nn = count(img==LOCALFILTER2D_IGNORE)
   !    print *,"count of dead pixels ",nn,"  (",nn*100.0/area,"%)"!GET RID OF
       print *,"" 
   end if
   
   !optional median filter implemented for comparison sake, previous salt & pepper filter should outperform this.
   if (median_filter>0) then
       print *,""
       print *,median_filter,"by",median_filter,"pixel median likelihood filter"
       !call median_filter_5x5(img,img_out)
       call median_filter_nxn(img,img_out,median_filter)
       img = min(1.0d0,max(0.0d0,img_out(0:Nx-1,0:Ny-1)))
   end if
   ...
   if (lambda/=LIB_CLA_NODEFAULT_R) then
       ...
       if ( (lambda /= LIB_CLA_NODEFAULT_R).and.(lambda>0) ) then
           ...
           !   old MaxLikelihood code                
           !th = bstd/lambda
           !call maxLikelihoodFilter( img,img_out, bstd,th )
           !   new max likelihood code 16/07/25
   ...
   !optional gaussian blur filter implemented for comparison sake, previous max likelihood filter should outperform this.
   if(gaussian_blur_filter>0.0d0) then
       print *,""
       print *,"guassian blur filter"
       !guassian blur filter here
       call gaussianBlurImage( img, gaussian_blur_filter,img_out )
       img = min(1.0d0,max(0.0d0,img_out(0:Nx-1,0:Ny-1)))
   end if
   ...
   if (flat) then
       ...
       call writeTables( 4.0d0,4.0d0,500 , "flat_GaussTables.dat" )
   !---    scale image?
       ...
       !---    determine feature size roughly using fourier transform
           ...
           call radialPowerFunction( img_fore(0:Mx-1,0:My-1) ,q_min,q_max, rpf,rpfsum,qrpfbar,q2rpfbar )  
           ...
           print *,""
           vRC = min( min(Mx,My)/3,ceiling(sigma * 2) )       !   std dev assumed to vary on scale 4 x characteristic scale of image
           if (vRC_in /= LIB_CLA_NODEFAULT_I) then
       ...
       !---    Buggy code, can have the problem that the sub images are outside the range of the full image
       !    Cx = max(1,Mx / vRC)
       !    Cy = max(1,My / vRC)
       ...
       !---    replacement, DRM 19/03/25
           call chunking( Mx,vRC,Cx )
           call chunking( My,vRC,Cy )
           print *,"variable Ridler-Calvard pixels in scaled image ",vRC," grid nodes required ",Cx,",",Cy
           allocate(bg_node(-1:Cx,-1:Cy))      !   defined -1:Cx for convenience- I'm going to extrapolate the edges.
           allocate(sd_node(-1:Cx,-1:Cy))      !    
           allocate(fg_node(-1:Cx,-1:Cy))      !   b
           ...
           do ky = 0,Cy-1        
               do kx = 0,Cx-1
                   ...
                   call progressBar( (kx+1) + Cx*ky , Cx*Cy )

               !   last chunk has the remaining pixels, which could be more than vRC
                   jx = (kx+1)*vRC ; if (kx==Cx-1) jx = Mx
                   jy = (ky+1)*vRC ; if (ky==Cy-1) jy = My
               !    print *,vRC,kx,ky,Cx,Cy,Mx,My,jx,jy
                   call findImageIntensityFeatures( img_fore( kx*vRC:jx-1 , ky*vRC:jy-1 ) , bb,th,ff,bstd,fbar,snr,fot ) 
                   bg_node(kx,ky) = bb
                   sd_node(kx,ky) = bstd
                   fg_node(kx,ky) = ff
               end do
       ...
       !*      h = 0 - foreground. h = 1 - background        
           do iy = 0,My-1
               ky = min( Cy-1,int(iy/vRC) )                    !   coarse node block  
               ay = iy/real(vRC) - (ky+0.5)                    !   distance to bg node             
               do ix = 0,Mx-1
                   kx = min( Cx-1, int(ix/vRC) )               !   coarse node block  
                   ax = ix/real(vRC) - (kx+0.5)                !     distance to bg node
   ...
   !       logistic = 0 = foreground 
       ...
       !---    last pass, this time on the full size image
           call radialPowerFunction( img ,q_min,q_max, rpf,rpfsum,qrpfbar,q2rpfbar )  
           sigma = sqrt(PI)/(2*qrpfbar)   
           print *,"single Gaussian sigma    = ",sigma
           print *,""
...
!                             
   ...
   subroutine median_filter_nxn( f_in, f_out ,n)
       !---^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
       !*      applies a nxn pixel media filter to an image using quadratic interpolation to pad the boundaries
               real(kind=real64),dimension(0:,0:),intent(in)               ::      f_in                                        !Image array in
               integer,intent(inout)                                          ::      n !window size, this needs to be odd
               real(kind=real64),dimension(:,:),allocatable,intent(out)    ::      f_out                                       !Blurred image out
               real(kind=real64),dimension(:,:),allocatable                ::      f_pad                                       !padded iamge of f_in to handle boundaries.
               real(kind=real64),dimension(:),allocatable                 ::      filter_kernel                                !flattened Kernel that the filter is applied per pixel.
               integer                                                     ::      Nx,Ny                                       !input image dimensions
               integer                                                     ::      Idx_min_X,Idx_min_Y ,Idx_max_X,Idx_max_Y    !min max idices of padded
               integer                                                     ::      ii,jj,kk,ll, kernel_count                                       !array indices
               integer                                                     ::      pad_length !how many pixels either side of the central pxiel, useful for iterpolation 
               integer                                                     ::      central_vector_element !of the filter_kernel to get median.
               



               !get input image dimesions
               Nx=size(f_in,dim=1)
               Ny=size(f_in,dim=2)

               !test for odd n
               if(modulo(n,2)==0) then
                   print*,"warning attempting to use an even window size (n), defaulting to 5x5"
                   n=5
               end if

               pad_length=(n-1)/2

               !allocate the filter_kernel and find the middle element
               allocate(filter_kernel(n*n))

               central_vector_element=(((n*n)-1)/2)+1



               !get padded image dimensions
               Idx_min_X=-pad_length
               Idx_min_Y=-pad_length
               Idx_max_X=Nx+pad_length
               Idx_max_Y=Ny+pad_length


               !allocate padded image
               allocate(f_pad(Idx_min_X:Idx_max_X,Idx_min_Y:Idx_max_Y))
               !get padded image
               call setExterior( f_in,EXTERIOR_EXTRAPOLATE,pad_length,f_pad )

               !allocate filtered image
               allocate(f_out(0:Nx-1,0:Ny-1))
               do ii=0,Nx-1
                   do jj=0,Ny-1
                       !get kernel values
                       kernel_count=0
                       do kk=-pad_length,pad_length
                           do ll=-pad_length,pad_length
                               kernel_count=kernel_count+1
                               filter_kernel(kernel_count)=f_pad(ii+kk,jj+ll)
                           end do
                       end do
                       !sort kernel
                       call quicksort(filter_kernel)
                       !get median value and assign to the associated output image
                       f_out(ii,jj)=filter_kernel(central_vector_element)

                   end do
               end do
               

       end subroutine median_filter_nxn



   subroutine chunking( N,p, C )            
!---^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
!*      split an integer number of pixels N
!*      into C blocks, each containing p pixels, 
!*      except for the last block which has p<=r<2p pixels.
       integer,intent(in)          ::      N,p
       integer,intent(out)         ::      C
       integer                 ::      rr      
        
       rr = p + mod(N,p)               !   number of pixels required for the last block
       if (rr==p) then             !   don't need a thin last block
           C = int( N/p )          !   I know p exactly divides N
       else                
           C = 1 + int( (N-rr)/p ) !   I know p exactly divides N-rr
       end if
       !print *,"chunking ",N," = ",C-1,"x",p," + ",rr," = ",(C-1)*p+rr
