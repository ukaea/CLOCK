...
!*    Copyright (C) UKAEA 2023 - 2025  Daniel Mason
...
!*              ( Dxy   Dyy )
!*              
!*      it is assumed that the background level is zero.
!*      note: has publically accessible components.
!*
!*      a gaussian is defined by its 6 parameters.
!*      Some routines ask for the array of length 6, in which case the order is
!*          ( x0,y0,f0,Dxx,Dxy,Dyy )
!*
!*      Version History
!*          2.0.0   Oct 2025        fitting background
...
!---        
    ...
    real(kind=real64),public                    ::      DIAM_THRESH = 1.0d0         !   discard any gaussian with minor diameter under this (NB in pixels)
    real(kind=real64),public                    ::      DIAM_THR_MAX = 1000.0d0      !   discard any gaussian with major diameter over this (NB in pixels)
    ...
    real(kind=real64),private,parameter         ::      UNREASONABLY_POOR_FIT = huge(1.0)/2
    integer,public,parameter                    ::      LIB_G2D_NLOOPS = 100
    real(kind=real64),public,parameter          ::      LIB_G2D_TOL = 1.0d-6
    real(kind=real64),public          ::      LIB_G2D_MAX_DIAM = 1.0d0    !how many times larger than the largest dimension of a given ROI can a Gaussian be?
...
!---
    ...
    subroutine fit0( this,img,imin,imax,mask ,ok, dbg)
!---^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
!*      anything not masked is out-of-bounds 
    ...
    !---    cut out the centre of the image, and set any points out of bounds                
        ...
        if (present(dbg)) then
            if (dbg) then
                print *,"Lib_Gaussians2d::fit0 info - range (",ix,"-",jx,"/",Nx,"  x  ",iy,"-",jy,"/",Ny," )"
                call fit_single_gaussian( this,img_clean,rss,dbg )
                call report(this)
            else
                call fit_single_gaussian( this,img_clean,rss )
            end if
        else
            call fit_single_gaussian( this,img_clean,rss )
        end if
        ...
        ok = (rss*2<UNREASONABLY_POOR_FIT)
    ...
    subroutine fit_single_gaussian( this,img,rss,dbg )
!---^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
!*      find the best fit gaussian to the intensity map
        type(Gaussian2d),intent(inout)                  ::      this
        real(kind=real64),dimension(0:,0:),intent(in)   ::      img
        ...
        real(kind=real64)               ::      rss1,rss2
        real(kind=real64),dimension(6)  ::      dat1

        integer                         ::      ii
        
        !integer,parameter               ::      LIB_G2D_NLOOPS = 100
        !real(kind=real64),parameter     ::      LIB_G2D_TOL = 1.0d-6
    ...
    !---    this is the null guess: max intensity, centred in the box        
        ...
        if (ignore(this,d=LIB_G2D_MAX_DIAM*max(Nx,Ny),dbg=present(dbg))) rss1 = UNREASONABLY_POOR_FIT
        dat1 = getDat(this) 
    ...
    !       this gives solvable analytic expressions.
        ...
        if (ignore(this,d=LIB_G2D_MAX_DIAM*max(Nx,Ny),dbg=present(dbg))) rss2 = UNREASONABLY_POOR_FIT
...
!*      on input, img should be cleaned up to have zero background level.
    ...
    !---    try to fit            
        do jj = 1,LIB_G2D_NLOOPS
            ...
            !print *,"refine_single_gaussian drss ",drss
...
!*      translate centre of gaussian
    ...
    logical function ignore0( this , q,t,d,dbg )
!---^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        type(Gaussian2d),intent(in)             ::      this
        real(kind=real64),intent(in),optional   ::      q,t         !   thresholds for quality, t-value, diameter
        real(kind=real64),intent(in),optional   ::      d           !   threshold for diameter (px), was INT
        logical,intent(in),optional             ::      dbg
        real(kind=real64)       ::      ee,dmaj,dmin,theta,dmax
        ...
        ignore0 = (ee>E_THRESH) 
        ignore0 = ignore0 .or. ((dmin*2)<DIAM_THRESH)       ! double to convert to diameter                        
        ignore0 = ignore0 .or. ((dmaj*2)>dmax)              ! double to convert to diameter                        
        ...
        if (present(dbg)) then
            if (dbg) then
                ...
                print *," Minor diam   ",2*dmin                                 ," < DIAM_THRESH (",DIAM_THRESH,")?  ",(2*dmin<DIAM_THRESH)
                print *," Major diam   ",2*dmaj                                 ," > DIAM_THR_MAX(",dmax,")?  ",(2*dmaj>dmax)
